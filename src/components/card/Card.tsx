...
const Card = (props: ICard) => {
    ...
    return (
        <div className={`${isDetail ? 'w-[56rem] h-96 delay-200' : 'w-72 h-80'} relative rounded-md overflow-hidden bg-white/5 ease-in-out duration-200`}>
            ...
            {!isDetail && (<label htmlFor="bulk-action" className="absolute top-0 right-0 p-4 cursor-pointer">
                <input id="bulk-action" defaultChecked={isDetail} type="checkbox" className="cursor-pointer" />
            </label>)}
            <div className={`${isDetail ? 'h-96 leading-6 w-full top-0 delay-100' : 'h-48 w-72 hover:top-20 hover:h-80 leading-9 hover:leading-6 hover:delay-200 top-56'} ease-in-out duration-300 absolute overflow-hidden p-4 bg-gradient-to-b from-green-700/90 via-green-800/90 to-slate-800`}>
